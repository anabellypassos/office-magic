            ...
            <aside className={`fixed lg:relative z-50 h-full w-80 bg-gradient-to-b from-docmind-dark to-slate-900 text-white flex flex-col shadow-2xl transition-all duration-500 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
                <div className="p-8 border-b border-slate-800 flex justify-between items-start">
                    <div className="flex flex-col">
                        ...
                        <span className="text-[10px] text-slate-500 uppercase tracking-[0.4em] mt-1 font-black">Intelligent Solutions</span>
                    </div>
                ...
                <div className="px-6 py-2">
                    <button onClick={handleCreateNew} className="w-full bg-gradient-to-r from-cyan-400 to-blue-600 text-white rounded-2xl py-5 text-[10px] font-black uppercase tracking-[0.2em] shadow-xl hover:scale-[1.02] active:scale-95 transition-all">+ New Document</button>
                </div>
                    ...
                    <ul className="space-y-2">
                        {documents.map((doc) => (
                            <li key={doc.id} onClick={() => { setSelectedDoc(doc); setView('editor'); setIsSidebarOpen(false); }} className={`group flex items-center justify-between p-4 rounded-2xl cursor-pointer transition-all ${selectedDoc?.id === doc.id && view === 'editor' ? 'bg-slate-800 text-docmind-accent border border-slate-700 shadow-xl' : 'text-slate-500 hover:bg-slate-800/30'}`}>
                                ...
                                <button onClick={(e) => handleDelete(doc.id, e)} className="lg:opacity-0 group-hover:opacity-100 text-red-400 p-2 hover:bg-red-500/10 rounded-lg">🗑️</button>
                            </li>
                        ))}
                    </ul>
                ...
                <div className="max-w-6xl mx-auto">
                    {view === 'calendar' ? (
                        ...
                        <div className="flex flex-col items-center justify-center h-full text-slate-200">
                            ...
                            <p className="text-slate-300 font-bold mt-4 uppercase tracking-[0.3em] text-xs">Intelligent Workspace</p>
                        </div>
                    )}
                </div>
            </main>
